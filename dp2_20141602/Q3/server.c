...
#define SERVER_NAME "cse20141602"
...
#define MAX_NET 1000000


pthread_mutex_t cli_work;
...
main( int argc, char** argv ){
	...
	if (argc != 3){
		printf("Argument number fail. Please check ./server [portnum] [num of threads] \n");
		return 0;
	}
	...
	*/
    ...
    if (listen(server_fd, LISTEN_LIMIT) == -1){
        perror("listen error : ");
        exit(0);
    }
    ...
    while(1)
    {
		/*
		   connect accept
		   */
        if ( (client_fd = accept(server_fd, (struct sockaddr *)&clientaddr, &client_len)) < 0){
            perror("Accept error : ");
            exit(0);
        }
...
void *background(void *aux)
{
	...
	while(1)
	{
		...
		while(cli_done_num >= cli_now_num){
			pthread_cond_wait(&cli_cond, &cli_work);
		}

		pthread_mutex_unlock(&cli_work);

		client_fd = cli_fd[cli_done_num++];
		...
		/*
			...
			*/
			...
			if ( fgets( line, sizeof(line), cli_fp ) == (char*) 0 )
			send_error( 400, "Bad Request", (char*) 0, "No request found." );
			if ( sscanf( line, "%[^ ] %[^ ] %[^ ]", method, path, protocol ) != 3 )
			send_error( 400, "Bad Request", (char*) 0, "Can't parse request." );


			while ( fgets( line, sizeof(line), cli_fp ) != (char*) 0 )
			{
			if ( strcmp( line, "\n" ) == 0 || strcmp( line, "\r\n" ) == 0 )
				break;
			}
